...
F = ff.probability_ode(x,g,A,G)
bool_param = 0
k_eq = 0 #step at which the equilibrium is reached 

for k in range(0, nrT-1): #nrT-1
    #controlled dynamic
    g_prec = g_MPC_value[k-1]
    opti.set_value(p,[q,x_s,x_MPC_value[k], g_max,h,g_prec,g_eq,delta_g])
    sol = opti.solve()
    ...
    if( bool_param==0 and x_s ==(float('%.3f'%(x_MPC_value[k+1])))):
       print('eq.reached for k =',(k+1), 'with h =', h)  
       k_eq = k+1
       bool_param = 1     
    #uncontrolled dynamic
    
#uncontrolled dymanics
...
plt.plot(time, g_normalized[1:(nrT-1)], label='Gain (normalized)', color='red',linewidth=3 )
